...
import { Activity, Users, Send, MousePointerClick, Zap, Calendar, Code2, Phone } from 'lucide-react';
...
const mockPieData = [
  { name: 'متصل',  value: 400, color: 'var(--success-500)' },
  { name: 'مفصول', value: 300, color: 'var(--danger-500)' },
  { name: 'معلق',  value: 300, color: 'var(--warning-500)' },
];

const recentActivity = [
  { text: 'تم إطلاق حملة جديدة "عروض الصيف"', meta: 'منذ ساعتين • حساب خدمة العملاء' },
  { text: 'تمت إضافة 3 حسابات جديدة',           meta: 'منذ 4 ساعات • النظام' },
  { text: 'اكتملت جدولة نشر 12 رسالة',          meta: 'أمس، 9:40 م • حساب التسويق' },
];
...
export default function DashboardHome({ accounts = [] }: { accounts?: any[] }) {
  ...
  const stats = [
    { title: 'إجمالي الحسابات',   value: accounts.length.toString(), icon: Users,             color: 'var(--info-500)',    change: '+12%', changeDirection: 'up' as const,  progress: 75 },
    { title: 'الرسائل المُرسلة', value: '1.2M',                      icon: Send,              color: 'var(--success-500)', change: '+5.4%',changeDirection: 'up' as const,  progress: 60 },
    { title: 'النقرات الإجمالية',value: '450K',                      icon: MousePointerClick, color: 'var(--brand-secondary)', change: '+18%', changeDirection: 'up' as const, progress: 85 },
    { title: 'Ping الخادم',       value: '24ms',                      icon: Zap,               color: 'var(--warning-500)', change: '-2ms', changeDirection: 'up' as const,  progress: 95 },
  ];
    ...
    <div className="flex flex-col gap-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-secondary mt-1 text-body-m">نظرة عامة على أداء حساباتك وحملاتك</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-heading-s text-primary">أداء الحملات</h3>
            ...
            <div className="h-[300px] w-full dir-ltr">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={mockData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  ...
                  <RechartsTooltip
                    contentStyle={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border-default)', borderRadius: '10px', color: 'var(--text-primary)', boxShadow: 'var(--shadow-lg)' }}
                    itemStyle={{ color: 'var(--text-primary)' }}
                  />
                  <Area type="monotone" dataKey="messages" name="رسائل" stroke="var(--brand-primary)" fillOpacity={1} fill="url(#colorMessages)" strokeWidth={2} />
                  <Area type="monotone" dataKey="clicks"   name="نقرات"  stroke="var(--brand-secondary)" fillOpacity={1} fill="url(#colorClicks)" strokeWidth={2} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card>
          <CardContent className="p-6">
            <h3 className="text-heading-s text-primary mb-6">حالة الحسابات</h3>
            <div className="h-[200px] w-full dir-ltr flex items-center justify-center relative">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <RechartsTooltip contentStyle={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border-default)', borderRadius: '10px', boxShadow: 'var(--shadow-lg)' }} />
                </PieChart>
              </ResponsiveContainer>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardContent className="p-6">
            <h3 className="text-heading-s text-primary mb-6">الأنشطة الأخيرة</h3>
        ...
        <div className="flex flex-col gap-4">
          <Card className="bg-gradient-to-br from-[rgba(0,168,132,0.1)] to-transparent border-[var(--brand-primary-light)] hover:border-[var(--brand-primary)] group cursor-pointer">
            <CardContent className="p-6 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-bold text-heading-s text-primary group-hover:text-[var(--brand-primary)] transition-colors">إرسال مباشر</h3>
                  <p className="text-sm text-secondary">إرسال رسالة فورية للمجموعات</p>
                </div>
              </div>
            </CardContent>
          </Card>
          <Card className="hover:border-[var(--brand-secondary)] group cursor-pointer">
            <CardContent className="p-6 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-bold text-heading-s text-primary">جدولة حملة</h3>
                  <p className="text-sm text-secondary">إعداد حملة للنشر لاحقاً</p>
                </div>
              </div>
            </CardContent>
          </Card>
