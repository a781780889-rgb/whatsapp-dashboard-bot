...
ALTER TABLE link_import_jobs ADD COLUMN IF NOT EXISTS new_joined INT DEFAULT 0;
ALTER TABLE link_import_jobs ADD COLUMN IF NOT EXISTS already_joined INT DEFAULT 0;
...
CREATE TABLE IF NOT EXISTS link_import_account_state (job_id UUID NOT NULL REFERENCES link_import_jobs(id) ON DELETE CASCADE, account_id UUID NOT NULL, status VARCHAR(30) DEFAULT 'idle', current_item_id BIGINT, processed INT DEFAULT 0, successful INT DEFAULT 0, new_joined INT DEFAULT 0, already_joined INT DEFAULT 0, failed INT DEFAULT 0, skipped INT DEFAULT 0, last_attempt_at TIMESTAMPTZ, last_error TEXT, updated_at TIMESTAMPTZ DEFAULT NOW(), PRIMARY KEY(job_id,account_id));
ALTER TABLE link_import_account_state ADD COLUMN IF NOT EXISTS new_joined INT DEFAULT 0;
ALTER TABLE link_import_account_state ADD COLUMN IF NOT EXISTS already_joined INT DEFAULT 0;
