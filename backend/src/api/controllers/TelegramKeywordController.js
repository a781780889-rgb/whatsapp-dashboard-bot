...
const Controller = {
  ...
  async ignore(req,res){ try { return res.json({success:true,...await Service.ignoreMessage(req.user.id,req.params.id)}); } catch(e){ console.error('[TelegramKeywordController.ignore]', { resultId: req.params.id, userId: req.user?.id, code: e.code, message: e.message }); return res.status(e.code === 'RESULT_NOT_FOUND' ? 404 : 500).json({success:false,error:e.message || 'تعذر تجاهل الرسالة',code:e.code || 'TELEGRAM_IGNORE_FAILED'}); } },
  async blockUser(req,res){ try { return res.json({success:true,...await Service.blockUser(req.user.id,req.params.id,req.body||{})}); } catch(e){ console.error('[TelegramKeywordController.blockUser]', { resultId:req.params.id, userId:req.user?.id, code:e.code, message:e.message }); const status=e.code==='RESULT_NOT_FOUND'||e.code==='SENDER_ID_MISSING'?404:500; return res.status(status).json({success:false,error:e.message||'تعذر حظر المستخدم بالكامل',code:e.code||'TELEGRAM_BLOCK_FAILED'}); } },
  async blockedUsers(req,res){ try { return res.json({success:true,users:await Service.listBlockedUsers(req.user.id,req.query||{})}); } catch(e){ return res.status(500).json({success:false,error:'تعذر تحميل المستخدمين المحظورين'}); } },
  async unblockUser(req,res){ try { return res.json({success:true,...await Service.unblockUser(req.user.id,req.params.id)}); } catch(e){ return res.status(e.code==='BLOCKED_USER_NOT_FOUND'?404:500).json({success:false,error:e.message||'تعذر إلغاء حظر المستخدم',code:e.code||'TELEGRAM_UNBLOCK_FAILED'}); } },
