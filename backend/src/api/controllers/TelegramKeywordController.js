...
const Controller = {
  ...
  async reply(req,res){ try { return res.json({success:true,...await Service.reply(req.user.id,req.params.id,req.body.text)}); } catch(e){ return res.status(400).json({success:false,error:e.message}); } },
