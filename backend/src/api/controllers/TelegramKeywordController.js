...
const Controller = {
  ...
  async ignore(req,res){ try { return res.json({success:true,...await Service.ignoreMessage(req.user.id,req.params.id)}); } catch(e){ return res.status(e.code === 'RESULT_NOT_FOUND' ? 404 : 400).json({success:false,error:e.message,code:e.code || 'TELEGRAM_IGNORE_FAILED'}); } },
