...
class LinkImportController {
  ...
  async job(req, res) { const job = LinkImportService.getJob(req.query.accountId, req.params.jobId); res.json({ success: !!job, job: job || null }); }
  async control(req, res) { try { const fn = { pause: 'pause', resume: 'resume', stop: 'stop' }[req.params.action]; if (!fn) throw new Error('إجراء غير صالح'); res.json({ success: true, job: LinkImportService[fn](req.body.accountId, req.params.jobId) }); } catch (e) { res.status(400).json({ success: false, error: e.message }); } }
}
