...
const Controller={
 async requestCode(req,res){try{return res.json({success:true,...await Auth.requestCode(req.user.id,req.body.phone)});}catch(e){return res.status(400).json({success:false,error:e.message});}},
