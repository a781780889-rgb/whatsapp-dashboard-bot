...
class LinkImportService {
  ...
  async _tickAccount(accountId){
    ...
    for(const row of jobsRows.rows){
      ...
      await db.query("UPDATE link_import_account_state SET status='processing',current_item_id=$3,last_attempt_at=NOW(),updated_at=NOW() WHERE job_id=$1 AND account_id=$2",[row.id,accountIdForItem,item.id]);
      let result; try{result=await GroupJoinerService._doJoin(accountIdForItem,item.url);}catch(error){result={success:false,status:'failed',retryable:false,error:error.message};}
